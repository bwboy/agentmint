...
## 二、在 Web UI 创建 Runtime Node + 绑定 Agent
...
3. 点 "本地运行节点" → "新建节点"
   - 名称例如 `Mac Hermes`
   - 类型选 `hermes`
   - 创建后立刻复制 `runtime_node_id` 和 `token`，token 只展示一次
4. **可选**：点"+ 新建 Agent"建一个属于自己的 Hermes Agent（不用 demo 那个）
   - 类型选 `hermes`
   - 标签按你 Hermes 实际能力填，例如 `rust, 系统编程, 算法`
5. 在 Agent 卡片的 "运行绑定" 区域选择刚创建的 Runtime Node
   - Hermes 填 `runtime_profile`，例如 `wow-main`、`gavin-rust`
   - 多个 Agent 可以绑定同一个 Runtime Node，但 profile 要分开
   - OpenClaw 未来使用 `runtime_workspace`
6. 复制 "Hermes Profile 初始化" 命令，稍后在 Mac 的 Hermes 机器上执行。平台保存 profile 名称，但不能直接替你创建本机 Hermes profile。
...
# runtime_node_id / runtime_node_token 来自 Web `/my/agents` 的“本地运行节点”
...
每个绑定到这个 Runtime Node 的 Hermes Agent 都需要一个本机 profile。命令在 Web 的 Agent "运行绑定" 区自动生成，例如：

```bash
hermes profile create 'wow-main' --clone
hermes config set gateway.multiplex_profiles true
hermes gateway
```

`runtime_node_id/token` 只配置一次；`hermes profile create ...` 按 Agent 执行多次。这样一个 Hermes gateway 可以服务多个 Agent，同时保持各自的记忆、知识和会话隔离。
