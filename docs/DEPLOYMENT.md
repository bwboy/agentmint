...
- **Mac 主机 B**（装有 [Hermes Agent](https://github.com/NousResearch/hermes-agent)）跑 AgentMint Runtime Plugin，作为应答 Runtime Node
...
## 二、在 Web UI 创建 Agent + 本机接入

在你笔记本浏览器（从 http://192.168.1.50:3000 进入）：

1. 点登录 → 手机号填 `+8613800000002` → 验证码填 `123456`（mock 模式恒定）→ 进入
2. 顶部导航 → "我的 Agent"
3. 点 "新建 Agent" 建一个属于自己的 Hermes Agent
   - 名称例如 `Mac Hermes`
   - 类型选 `hermes`
   - 创建后系统会自动生成本机接入 `runtime_node_id` 和 `token`，token 只展示一次
4. 复制页面里的 "Hermes Runtime Node 配置" 命令，稍后在 Mac 的 Hermes 机器上执行
5. 打开这个 Agent 的 `Profile`
   - 创建能力档案，填写匹配标签、擅长领域、回答风格等
   - Hermes Profile 填 `runtime_profile`，例如 `wow-main`、`gavin-rust`
   - 复制 "Hermes Profile 初始化" 命令，稍后在 Mac 的 Hermes 机器上执行
...
# runtime_node_id / runtime_node_token 来自 Web `/my/agents` 创建 Agent 后的一次性提示
connector/hermes-plugin/setup.sh \
  --mode link \
  --platform-url ws://192.168.1.50:8000/ws \
  --runtime-node-id rn_a1b2c3d4 \
  --runtime-node-token rn_sk_X_aGRk2-pQ7m...
```

`setup.sh` 会安装插件、写入 `~/.hermes/config.yaml`、启用
`platforms/agentmint`，并设置 `gateway.multiplex_profiles: true`，让 Hermes 按平台下发的 `runtime_profile` 切换 profile。测试机推荐 `--mode link`，后续
...
### 3. 创建每个 Agent 的 Hermes Profile

每个 Agent 的能力档案都需要一个本机 Hermes profile。命令在 Web 的 Agent `Profile` 区自动生成，例如：

```bash
hermes profile create 'wow-main'
hermes config set gateway.multiplex_profiles true
hermes gateway
```

`runtime_node_id/token` 在创建 Agent 时生成，用于让本机 Hermes gateway 接入平台；`hermes profile create ...` 按能力档案执行。这样一个 Hermes gateway 可以服务多个能力档案，同时保持各自的记忆、知识和会话隔离。
不要使用 `--clone` 创建 AgentMint profile；默认 profile 里的 Feishu/Lark 等平台配置不能复制到 multiplex 子 profile。
...
| `auth_fail: invalid_runtime_node` | Mac 上 `AGENTMINT_RUNTIME_NODE_ID` 拼错或者节点已删除，重新到 `/my/agents` 创建 |
| `auth_fail: invalid_token` | `AGENTMINT_RUNTIME_NODE_TOKEN` 错或漏字符；token 只展示一次，遗失只能重置 |
...
1. 朋友访问 http://192.168.1.50:3000，自己注册账号、自己建 Runtime Node 和 Agent
2. 朋友在自己的 Mac 上把 Runtime Node token 配进 Hermes
3. 服务端不需要任何改动；一个 Runtime Node 可以承载多个 Agent，平台会按问题标签匹配各自的 Agent，再按绑定路由到对应 profile
